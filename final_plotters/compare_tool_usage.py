...
class ModelToolComparer:
    ...
    def load_data(self):
            ...
            for line in f:
                try:
                    ...
                    if "model_performance" in entry and "tool_call_lists" in entry:
                        problem_id = entry.get(
                            "function_id", entry.get("problem", "unknown")
                        )
                        ...
                        for model_name, configs in entry["model_performance"].items():
                            if self.config_name in configs:
                                ...
                                tool_calls = []
                                if (
                                    model_name in entry["tool_call_lists"]
                                    and self.config_name
                                    in entry["tool_call_lists"][model_name]
                                ):
                                    tool_calls = entry["tool_call_lists"][model_name][
                                        self.config_name
                                    ]
        ...
        print(
            f"Loaded {len(self.data)} problem-model combinations for config '{self.config_name}'"
        )
    ...
    def analyze_tool_usage_distribution(self):
        ...
        for model, counts in model_tool_counts.items():
            ...
            normalized_counts[model] = {
                tool: count / total_problems for tool, count in counts.items()
            }
    ...
    def plot_tool_usage_heatmap(self, normalized_counts):
        ...
        try:
            ...
            sns.heatmap(
                ...
                cmap="PuRd",
                fmt=".2f",
                cbar_kws={"label": "Average Usage Per Problem"},
            )
            plt.title(
                f"Tool Usage Distribution Across Models for {self.config_name.split('_')[0].capitalize()}"
            )
            plt.ylabel("Model")
    ...
    def analyze_average_info_and_submissions(self):
        ...
        for model in self.models:
            ...
            for problem in self.model_problems[model]:
                ...
                submit_count = sum(
                    1
                    for t in problem["tool_usage"]
                    if t.get("tool") == "submit_attempt"
                    or t.get("tool") == "replace_function"
                )
                submission_counts.append(
                    min(submit_count, self.test_budget)
                )  # Cap at test budget
                ...
                info_count = sum(
                    ...
                    for t in problem["tool_usage"]
                    if t.get("tool") != "submit_attempt"
                    and t.get("tool") != "replace_function"
                )
    ...
    def plot_average_info_and_submissions(self, model_stats):
        ...
        models = sorted(
            model_stats.keys(), key=lambda m: model_stats[m]["avg_submissions"]
        )
        ...
        [model_stats[model]["problem_count"] for model in models]
        ...
        plt.title(
            f"Information Gathering Tool Usage vs Submissions for {self.config_name.split('_')[0].capitalize()}"
        )
        ...
        plt.tight_layout()
        output_file = (
            self.output_dir / f"avg_info_and_submissions_{self.config_name}.png"
        )
        plt.savefig(output_file, dpi=300, bbox_inches="tight")
    ...
    def plot_info_to_submission_ratio(self, model_stats):
        ...
        for model in models:
            ...
            ratio = (
                stats["avg_info_gathering"] / stats["avg_submissions"]
                if stats["avg_submissions"] > 0
                else 0
            )
            ratios.append(ratio)
