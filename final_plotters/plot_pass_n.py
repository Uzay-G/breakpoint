                ...
                if config_name in data.get("tool_call_lists", {}).get(model_name, {}):
                    ...
                    if score == 1.0:
                        ...
                        if len(submit_indices) != 0:
                            print(
                                len(submit_indices),
                                "submit attempts found but not solved",
                            )
...
def plot_multi_model_pass_at_n(
    pass_at_n_by_model,
    model_totals,
    config_name,
    output_file="pass_at_n_multi_model.png",
):
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python pass_at_n_multi_model.py <path_to_jsonl_file> [config_name]"
        )
        sys.exit(1)
    ...
    pass_at_n_by_model, model_totals = calculate_aggregated_pass_at_n(
    ...
    plot_multi_model_pass_at_n(
        pass_at_n_by_model, model_totals, config_name, output_file
    )
