...
def prepare_correlation_data(data):
    ...
    for item in data:
        ...
        metrics["code_line_count"] = complexity.get(
            "code_line_count", complexity.get("line_count", None)
        )
...
def create_correlation_heatmap(df, output_path=None):
    ...
    # Customize the plot
    ax.set_title(
        "Correlation Matrix of Code Complexity and Centrality Metrics",
        fontsize=18,
        pad=20,
    )
    ...
    group_positions.append(
        (
            current_pos
            + len([col for col in success_metrics if col in ordered_cols]) / 2,
            "Success",
        )
    )
...
def calculate_metric_importance(df):
    ...
    feature_cols = [
        ...
        if col not in ["function_id", "avg_success", "max_success", "min_success"]
    ]
...
def main():
    ...
    print(
        f"Metrics included: {', '.join([col for col in df.columns if col != 'function_id'])}"
    )
